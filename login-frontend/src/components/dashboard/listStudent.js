...
function ListStudent() {
  ...
  const getData = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        console.log("All student(s):", result);
  ...
  const handleAdd = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/admin/dashboard/student/add`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${jwtToken}`, // Include the token in the request header
          "Content-Type": "application/json",
  ...
  const handleEdit = (row) => {
    //alert('Edit student information');
  };
  const handleDelete = async (row) => {
    //event.preventDefault();
    
    let studentid = row.original.mssv;
    try {
      const response = await fetch(`http://localhost:5000/admin/dashboard/student/${studentid}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${jwtToken}`, // Include the token in the request header
          "Content-Type": "application/json",
    ...
    () => [
      {
        ...
        muiEditTextFieldProps: {
          type: 'text',
          required: true,
          error: !!validationErrors?.mssv,
          helperText: validationErrors?.mssv,
          onFocus: () =>
            setValidationErrors({
              ...validationErrors,
              mssv: undefined,
...

      {
        ...
        header: 'Edit',
        Cell: ({ row }) => (
          // <a onClick={() => handleNavigation('EditStudent')}>
    ...
    <>
      <div className="dataTable mx-auto">
        ...
        <div className='mb-3' style={{ backgroundColor: '#f2f2f2', padding: '10px' }}>
          ...
          <Button style={{ marginLeft: '65%', backgroundColor: '#20bd20', borderColor: 'green' }} onClick={() => handleShowModal()}>Add student</Button>
        </div>
